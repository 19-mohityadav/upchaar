...
import { fetchBloggers } from '@/lib/adminApi.js';
import { supabase } from '@/lib/supabase.js';
import { Search, Plus, Trash2, PenLine, Mail, X, ChevronLeft, ChevronRight } from 'lucide-react';
import { cn } from '@/lib/utils';
import { format } from 'date-fns';
import { motion, AnimatePresence } from 'framer-motion';

const PAGE_SIZE = 8;
const EMPTY_FORM = { name: '', email: '', specialty: '', bio: '', avatarColor: '#0d9488' };
...
export default function BloggerManagement() {
    ...
    const loadBloggers = async () => {
        try {
            setLoading(true);
            const data = await fetchBloggers();
            setBloggers(data);
        } catch (err) {
    ...
    const handleAdd = async (e) => {
        ...
        try {
            ...
            const { data, error } = await supabase.from('controllers').insert([newBlogger]).select().single();
            if (error) throw error;
            setBloggers(prev => [{ ...data, posts: 0 }, ...prev]);
            setShowAdd(false);
            setForm(EMPTY_FORM);
            setPage(1);
            showToast(`${data.name} added as Blogger ✓`);
        } catch (err) {
            showToast(err.message, 'error');
        }
    };
        ...
        <div className="space-y-5">
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-slate-500 mt-0.5">{bloggers.length} bloggers registered</p>
                </div>
                {isSuperAdmin && (
                ...
                <table className="w-full text-sm">
                    <thead>
                        <tr className="border-b border-slate-100 bg-slate-50">
                            {['Blogger', 'Email', 'Specialty', 'Posts', 'Joined', 'Status', ...(isSuperAdmin ? ['Actions'] : [])].map(h => (
                    ...
                    <tbody className="divide-y divide-slate-50">
                        {paged.map(blogger => (
                            <tr key={blogger.id} className="hover:bg-slate-50/70 transition-colors">
                                <td className="px-4 py-3">
                                    <div className="flex items-center gap-3">
                                        <div className="h-9 w-9 rounded-full flex items-center justify-center font-bold text-white text-sm flex-shrink-0"
                                            style={{ backgroundColor: blogger.avatarColor }}>
                                            {blogger.name.replace('Dr. ', '')[0]}
                                        </div>
                                        <div>
                                            <p className="font-medium text-slate-800">{blogger.name}</p>
                                            <p className="text-xs text-slate-400">{blogger.id}</p>
                                        </div>
                                    </div>
                                </td>
                                <td className="px-4 py-3">
                                ...
                                <td className="px-4 py-3 text-slate-500 text-xs whitespace-nowrap">{format(new Date(blogger.created_at || new Date()), 'dd MMM yyyy')}</td>
                                <td className="px-4 py-3">
                                    <span className="px-2.5 py-1 rounded-full text-xs font-semibold border bg-emerald-50 text-emerald-600 border-emerald-200">{blogger.status}</span>
                                </td>
                                {isSuperAdmin && (
                                    <td className="px-4 py-3">
                        ...
                        {paged.length === 0 && (
                            <tr><td colSpan={isSuperAdmin ? 7 : 6} className="px-4 py-12 text-center text-slate-400 text-sm">No bloggers found</td></tr>
                        )}
                    </tbody>
                </table>
                {totalPages > 1 && (
                    ...
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                        ...
                        <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
                            ...
                            <form onSubmit={handleAdd} className="p-5 space-y-4">
                                {[
                                    { key: 'name', label: 'Full Name', placeholder: 'Dr. Meera Krishnan', type: 'text' },
                                    { key: 'email', label: 'Email', placeholder: 'blogger@sanjiwani.health', type: 'email' },
                                    { key: 'specialty', label: 'Medical Specialty', placeholder: 'General Medicine', type: 'text' },
                                    { key: 'bio', label: 'Bio (optional)', placeholder: 'Short bio…', type: 'text' },
                                ].map(({ key, label, placeholder, type }) => (
                                    <div key={key}>
                                        <label className="block text-xs font-semibold text-slate-600 mb-1.5">{label}</label>
                                        <input type={type} value={form[key]} onChange={e => setForm(f => ({ ...f, [key]: e.target.value }))}
                                            placeholder={placeholder} required={key !== 'bio'}
                                ...
                                <div className="flex gap-3 pt-1">
                                    ...
                                    <button type="submit"
                                        className="flex-1 py-2.5 rounded-xl bg-gradient-to-r from-primary to-teal-500 text-white text-sm font-semibold hover:opacity-90 transition">Add Blogger</button>
                                </div>
                            </form>
                        </motion.div>
                    </motion.div>
                ...
                {deleteTarget && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                        ...
                        <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }}
                            ...
                            <h3 className="font-bold text-slate-800 text-lg mb-1">Remove Blogger?</h3>
