...
import { Save, Loader2, CheckCircle, User, Phone, Mail, MapPin, Stethoscope, Clock } from 'lucide-react';
    ...
    return (
        <div className="space-y-6 max-w-3xl">
            <div>
                ...
                <p className="text-sm text-slate-500 mt-0.5">Update your professional details and availability</p>
            </div>
            ...
            <form onSubmit={handleSave} className="space-y-6">
                ...
                <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
                    ...
                    <div className="flex items-center gap-4">
                        <div className="h-16 w-16 rounded-2xl flex items-center justify-center font-bold text-white text-xl shadow-md"
                ...
                <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        {[
                            ...
                            { name: 'clinicName', label: 'Clinic / Hospital', placeholder: 'Heart Care Clinic' },
                            { name: 'experience', label: 'Experience (years)', placeholder: '10', type: 'number' },
                            { name: 'fee', label: 'Consultation Fee (₹)', placeholder: '1500', type: 'number' },
                        ...
                        <div className="flex gap-4">
                            {[{ name: 'hoursFrom', label: 'From' }, { name: 'hoursTo', label: 'To' }].map(({ name, label }) => (
                                <div key={name} className="flex-1">
                                    <label className="block text-xs font-semibold text-slate-600 mb-1.5">{label}</label>
                                    <input type="time" name={name} value={form[name]} onChange={handleChange}
                                        className="w-full h-10 px-3 rounded-xl border border-slate-200 bg-slate-50 text-sm focus:outline-none focus:ring-2 focus:ring-primary/25" />
                                </div>
                            ))}
                        </div>
                ...
                <button type="submit" disabled={saving}
                    className={cn('flex items-center gap-2 px-8 py-3 rounded-xl font-semibold text-sm transition-all shadow-md',
